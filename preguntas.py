...
def pregunta_01():
    ...
    y_reshaped = np.array(y).reshape(np.array(y).shape[0],1)

    # Trasforme `X` a un array de numpy usando reshape
    X_reshaped = np.array(x).reshape(np.array(x).shape[0],1)
...
def pregunta_02():
    ...
    # Imprima la correlación entre las columnas `` y `fertility` con 4 decimales.
